...
const EditEventDay = ({eventDayId,reload}) => {
    ...
    const submit = async (e) => {
        ...
        if(eventDay) {
            setLoading(true);
            request('PUT','event/day',eventDay,null,true)
            .then((response) => {
                setLoading(false);
                if(response.status) {
    ...
    const inputs = [
        {
            ...
            type:'text', 
            name:'event',
            value:eventDay && eventDay.event?eventDay.event.name:'',
            disabled:true
        },
        {
            label:'Date',
    ...
    useEffect(() => {
        request('GET',`event/day/${eventDayId}`,null,null,false)
        .then((response) => {
            if(response.content) {
                if(response.content.date) {
                    response.content.date = new Date(response.content.date);
                }
                setEventDay(response.content);
            }
        })
    },[])
    return (
        <FormDialog title='Edit Event Day'>
