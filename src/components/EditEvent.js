...
const EditEvent = ({id,reload}) => {
    ...
    const submit = async (e) => {
        ...
        setLoading(true);
        request('PUT','event',event,null,true)
        .then((response) => {
            setLoading(false);
            if(response.status) {
    ...
    useEffect(() => {
      (async () => {
          if(id) {
            let evnt = null;
            await request('GET',`event/${id}`,null,null,false)
            .then((response) => {
                if(response.content) {
                    evnt = response.content;
                } 
            })
              ...
              .then((response) => {
                  if(response.status) {
                      if(response.status === 'SUCCESSFUL' && response.content && response.content.length > 0) {
                          ...
                          evnt.criteriaPathItem = response.content[0];
                      } else {
                          setCriteriaPaths([]);
                      }
                  } else  {
